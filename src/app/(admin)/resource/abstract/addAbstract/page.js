...
export default function AddAbstract() {
  ...
  const handleAbstractTitleChange = (e) => {
    setAbstractTitle(e.target.value);
  };
  ...
  const submitForm = async () => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/abstract/add`, {
        method: 'POST',
        headers: {
        ...
        <div className="grid gap-4 mb-4 md:grid-cols-2">
          <div>
            ...
            <input
              type="text"
              value={abstractTitle}
              onChange={handleAbstractTitleChange}
              id="abstractTitle"
