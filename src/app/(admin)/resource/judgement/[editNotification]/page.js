...
export default function EditJudgement({ params }) {
  ...
  const policyId = params?.editNotification; // Adjusted to match the parameter name for policy
  const token = Cookies.get("token");

  useEffect(() => {
    const fetchPolicyDetails = async () => {
      try {
        const res = await fetch(`${API_BASE_URL}/judgement/JudgementById/${policyId}`, {
          headers: {
        ...
        if (data.success) {
          const judgement = data.data;
          setNotificationTitle(judgement.Title);
          setPdf(judgement.PDF);
        } else {
          toast.error(data.errMessage || "Failed to fetch judgement details");
        }
      } catch (error) {
        toast.error("An error occurred while fetching judgement details");
      }
    };
  ...
  const submitForm = async () => {
    ...
    try {
      setLoading(true);
      const res = await fetch(`${API_BASE_URL}/judgement/updateJudgement/${policyId}`, {
        method: 'PATCH',
        headers: {
          'Authorization': `Bearer ${token}`,
