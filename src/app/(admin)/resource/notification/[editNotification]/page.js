...
export default function EditNotification({ params }) {
  ...
  const notificationId = params.editNotification;
  ...
  useEffect(() => {
    ...
    const fetchNotification = async () => {
      ...
      try {
        const res = await fetch(`${API_BASE_URL}/notification/NotificationById/${notificationId}`, {
          headers: {
            "Authorization": `Bearer ${token}`,
          },
        });
        ...
        if (data.success) {
          ...
          toast.error(data.errMessage || "Failed to fetch notification");
        }
      } catch (error) {
        console.error(error);
        toast.error("An error occurred while fetching the notification");
      } finally {
  ...
  const submitForm = async () => {
    ...
    try {
      const res = await fetch(`${API_BASE_URL}/notification/updateNotification/${notificationId}`, {
        method: 'PATCH',
        headers: {
          "Authorization": `Bearer ${token}`,
        },
      ...
      if (data.success) {
        ...
        toast.error(data.errMessage || "Failed to update notification");
      }
    } catch (error) {
      console.error(error);
